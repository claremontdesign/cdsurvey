...
class AnswerOption extends Repository
{
	 ...
	 * @param ModelAnswer $answerType
	 * @return Model
	 */
	public function byAnswerType($answerType, $enabled = true)
	{
		$filters = [
			$this->_table() . '.answer_type_id' => $answerType->id()
		];
		if($enabled)
		{
			$filters[$this->_table() . '.status'] = 1;
	...
	{
		$notColumns = ['survey_id', 'question_id'];
		if(!empty($filters))
		{
